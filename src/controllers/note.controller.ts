...
class NoteController {
  ...
  public getAllNotes = async (
    ...
    try {
      ...
      res.status(HttpStatus.OK).json({
        code: HttpStatus.OK,
        message:
          data.length > 0 ? 'Notes fetched successfully' : 'No notes found',
        data
      });
    } catch (error) {
      next(error);
  ...
  public TrashNote = async (
    ...
    try {
      ...
      if (
        error.message ===
        'Note is archived and cannot be trashed. Unarchive it first.'
      ) {
        res.status(HttpStatus.BAD_REQUEST).json({
          code: HttpStatus.BAD_REQUEST,
          message: error.message
